...
# Statefata_sum <- aggregate(list(sumFATA=totalstate_FI$FATALITIES), list(STATE=totalstate_FI$STATE), sum, rm.na=TRUE)
```
...
# Stateprop_sum <- aggregate(list(sumPROP=totalstate_PC$PROPCOST), list(STATE=totalstate_PC$STATE), sum, rm.na=TRUE)
...
```{r statesY, echo=TRUE}
        ...
        count_Y <- ddply(stormSubsetdf, .(YEAR = format(BGN_DATE, "%Y"), EVTYPE), summarise, 
                         FATALITIES=sum(FATALITIES, na.rm = TRUE), 
                         INJURIES = sum(INJURIES, na.rm = TRUE),
                         PROPERTY  = sum(PROPCOST, na.rm = TRUE), 
                         CROP  = sum(CROPCOST, na.rm = TRUE))
...
ddply(stormSubsetdf,.(YEAR = format(BGN_DATE, "%Y")),summarise,count = length(EVTYPE))
storm_EY <- ddply(stormSubsetdf,.(YEAR = format(BGN_DATE, "%Y"),EVTYPE),summarise,count = length(EVTYPE))
